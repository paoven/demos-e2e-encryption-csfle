output "resource-ids" {
  ...
  Schema Registry URL: ${confluent_schema_registry_cluster.advanced.rest_endpoint}
  ...
  --property schema.registry.url=${confluent_schema_registry_cluster.advanced.rest_endpoint} \
  ...
  kafka-avro-console-consumer --group confluent_cli_consumer_encrypted_fields --topic ${confluent_kafka_topic.orders.topic_name}  --bootstrap-server   ${confluent_kafka_cluster.standard.bootstrap_endpoint}   --property schema.registry.url=${confluent_schema_registry_cluster.advanced.rest_endpoint} --property basic.auth.user.info="${confluent_api_key.csfle-app-consumer-encrypted-schema-registry-api-key.id}:${confluent_api_key.csfle-app-consumer-encrypted-schema-registry-api-key.secret}" --property basic.auth.credentials.source=USER_INFO --from-beginning --consumer-property security.protocol=SASL_SSL --consumer-property sasl.mechanism=PLAIN --consumer-property sasl.jaas.config="org.apache.kafka.common.security.plain.PlainLoginModule required username=\"${confluent_api_key.csfle-app-consumer-encrypted-kafka-api-key.id}\" password=\"${confluent_api_key.csfle-app-consumer-encrypted-kafka-api-key.secret}\";"
